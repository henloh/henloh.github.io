sap.ui.define(["./BaseController", "../model/formatter", "sap/ui/model/json/JSONModel", "./Types", "sap/m/Panel", "sap/m/Select", "sap/ui/core/Item", "sap/m/Title", "sap/m/Button"], function (__BaseController, __formatter, JSONModel, ___Types, Panel, Select, Item, Title, Button) {
  ...
  class ProductionLine extends BaseController {
    ...
    async generateProductionLine(event) {
      ...
      var game = new Game(this.getModel("GoodModel").getProperty("/Goods"), this.getModel("FactorieModel").getProperty("/Factories")); // avoid duplicate ID

      var Panelcount = 0;
      var currentPath = "";
      var viewModel = this.getModel("View"); // remove previos items 
      ...
      deathArray = []; // list of total required factories
      ...
      var getFactoryMaterialPanels = function (good, currentPath, factory) {
        var materialarray = game.getMaterialsForFactory(factory.Name); //console.log(materialarray);
      ...
      var getFactoryOrSelectWithPanel = function (good, isParentActive) {
        ...
        Panelcount++;
        var factories = game.getFactoriesForProduct(good.Name); //console.log(factories);
        ...
        if (factories.length == 1) {
          ...
          var path = currentPath + "/" + good.factoryOptions[0].Name.replace(/\s/g, ''); // verhindern von endlos rekursion (Mine braucht Borher, Bohrer braucht Stahl, Stahl braucht Mine)
          ...
          deathArray.push(button); // ermittle welche fabrik die "einfachsten" komponenten hat
      ...
      for (var treeitem of finalTree) {
        treeitem = getFactoryOrSelectWithPanel(treeitem, true); // var panel = getPanelWithSelect(good.Name)
      ...
      viewModel.setProperty("/prodTree", finalTree); //console.log(viewModel.getProperty("/tree"));
    ...
    selectFactory(event) {
      ...
      var getFlat = function (material) {
        try {
          ...
          console.warn("No factory"); // console.warn(error)
        }
      };
    ...
    setBaseFactory(event) {
      var viewModel = this.getModel("View"); // loop base products!
      // get into details
      ...
      var Material = viewModel.getProperty(path); // erstes control ist immer das select da es den button nur bei Produkten gibt die mehrere hersteller haben
